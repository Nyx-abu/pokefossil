    ...
    return (
        ...
        >
            <div className={`w-full max-w-2xl border-4 border-dashed rounded-xl p-6 md:p-12 text-center transition-colors flex flex-col items-center justify-center cursor-pointer relative ${isHovering ? 'border-[var(--color-gba-screen-text)] bg-black/10' : 'border-[var(--color-gba-screen-text)]/40 hover:border-[var(--color-gba-screen-text)]'}`}>
                ...
                />
                <div className="text-xl md:text-2xl mb-4 font-bold text-[var(--color-gba-screen-text)]">Drop a .sav file</div>
                <div className="text-sm md:text-base text-[var(--color-gba-screen-text)] opacity-80">or click to browse</div>
            </div>

            <div className="mt-8 text-center text-[var(--color-gba-screen-text)]">
                <p className="text-xs md:text-sm font-bold opacity-90">Runs entirely in your browser. Nothing is uploaded.</p>
                <p className="text-[10px] md:text-xs opacity-70 mt-2 font-bold">Supports: Ruby · Sapphire · Emerald · FireRed · LeafGreen</p>
                {error && <p className="text-red-700 font-bold mt-4 bg-red-100/50 p-2 rounded">{error}</p>}
            </div>
        </div>
    );
};
