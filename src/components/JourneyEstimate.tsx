    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            <div className="flex justify-between items-center border-b border-gray-700 pb-3 mb-3 flex-shrink-0">
                ...
                <h1 className="text-lg md:text-xl font-bold font-serif">Journey Estimate</h1>
            </div>
            ...
            <div className="flex-grow overflow-x-auto overflow-y-hidden whitespace-nowrap scrollbar-thin py-6 px-4 relative flex items-center min-h-0">
                ...
                <div className="flex space-x-12 items-center">
                    <div className="flex flex-col items-center justify-center">
                        <div className="w-28 h-28 rounded-lg border-2 border-dashed border-gray-700 flex items-center justify-center mb-2 bg-white/40">
                            <span className="text-xs font-serif text-gray-500">START</span>
                        </div>
                        <div className="w-3.5 h-3.5 bg-gray-500 rounded-full mb-1"></div>
                        <span className="font-bold text-xs text-gray-500 font-serif">New Game</span>
                    </div>
                    ...
                    {timeline.map((event, i) => (
                        <div key={i} className="flex flex-col items-center justify-center relative group" style={{ opacity: getOpacity(event.confidence) }}>
                            <div className="w-28 h-28 rounded-lg bg-[#f4f1ea]/80 border border-gray-700 flex items-center justify-center p-1 mb-2 bg-[#f4f1ea]/80 shadow-inner border-gray-700 group-hover:border-[var(--color-brand-accent)] transition-all">
                                <PokemonSprite 
                            ...
                            <span className="font-bold text-xs font-serif max-w-[120px] truncate text-center">{event.pokemon ? getDisplayName(event.pokemon.nickname, event.pokemon.species) : 'Unknown'}</span>
                            <span className="text-[10px] text-gray-500 font-serif">({event.confidence})</span>
                            
                            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/95 border border-gray-700 p-2 rounded text-xs z-30 whitespace-normal w-52 text-center pointer-events-none shadow-xl font-serif">
                                ...
                                <div className="text-gray-500 text-[11px]">{event.description}</div>
